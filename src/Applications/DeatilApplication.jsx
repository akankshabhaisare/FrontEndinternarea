...
useEffect(()=>{
  ...
  const response=await axios.get(`https://internarea-1.onrender.com//api/application/${id}`)
...
const handleAcceptAndReject= async(id,action)=>{
  try {
    const response=await axios.put(`https://internarea-1.onrender.com//api/application/${id}`,{action})
